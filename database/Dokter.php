...
class Dokter{
    ...
    function __construct(){
        try{
            $this->pdo = new PDO('mysql:host=localhost;dbname=db_pemweb','root','');
    ...
    public function tambahDokter($nama, $email, $telp, $pass, $spesialis, $foto, $date, $jk, $deskripsi){
        $sql = "INSERT INTO tuser (nama_user, email_user, telp_user, pass_user, spesialis, role, foto_user, tgl_lahir, jk, deskripsi) VALUES(?,?,?,?,?,?,?,?,?,?)";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([
            $nama, $email, $telp, $pass, $spesialis, 'dokter', $foto, $date, $jk, $deskripsi
        ]);
    }
    ...
    public function updateDokter($id, $nama, $telp, $pass, $spesialis, $foto, $deskripsi){
        $sql = "UPDATE tuser SET nama_user=?, telp_user=?, pass_user=?, spesialis=?, foto_user=?, deskripsi=? WHERE id_user=?";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([
            $nama, $telp, $pass, $spesialis, $foto, $deskripsi, $id
        ]);
        return 1;
    }

    public function updateDokterBio($id, $nama, $tgl_lahir, $telp, $jk){
        $sql = "UPDATE tuser SET nama_user=?, tgl_lahir=?, telp_user=?, jk=? WHERE id_user=?";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([
    ...
    public function updateUserPass($id, $pass){
        $sql = "UPDATE tuser SET pass_user=? WHERE id_user=?";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([
            $pass, $id
        ]);
