...
$pass = $_POST['password'];
$deskripsi = $_POST['deskripsi'];  // Add this line to get the description
...
if ($dktr->checkEmail($email) == 1) {         //check email
    ...
    if ($filesize <= $sizemax) {
        ...
        if (in_array($ext, $extboleh)) {
            // Pass the 'deskripsi' parameter here
            $dktr->tambahDokter($nama, $email, $telp, $passenkripsi, $spesialis, $alamat, $date, $jk, $deskripsi);
